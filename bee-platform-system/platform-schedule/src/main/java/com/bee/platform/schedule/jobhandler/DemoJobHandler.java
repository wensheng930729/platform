...
public class DemoJobHandler extends IJobHandler {
	...
	public ReturnT<String> execute(String param) throws Exception {
		...
		// 测试Feign客户端的调用
		ResponseResult<String> userBeatResult = userBeatFeignClient.beat();
		log.info("userBeatResult=" + userBeatResult);
		ResponseResult<String> businessResult = businessBeatFeignClient.beat();
		log.info("businessBeatFeignClient=" + businessResult);
		for (int i = 0; i < 5; i++) {
