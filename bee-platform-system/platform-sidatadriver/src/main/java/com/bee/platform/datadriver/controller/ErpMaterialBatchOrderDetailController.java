...
public class ErpMaterialBatchOrderDetailController {
    ...
    public ResponseResult<List<ErpMaterialBatchProductDTO>> getMaterialBatchDetails(@RequestParam Integer id) {
        if (ObjectUtils.isEmpty(id)) {
            log.info("参数异常");
            return ResponseResult.buildResponseResult(ResCodeEnum.ERROR_PARAMETER);
        }
        List<ErpMaterialBatchProductDTO> dto = erpMaterialBatchOrderDetailService.getMaterialBatchDetails(id);
