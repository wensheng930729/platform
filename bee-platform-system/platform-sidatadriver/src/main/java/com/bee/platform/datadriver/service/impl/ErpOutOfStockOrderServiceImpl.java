...
//        rq.setList(ids);
        ...
        List<ErpOutOfStockOrderSearchListDTO> dto = baseMapper.searchOutOfStockOrderByCondition(rq,pagination);
        ...
        if(!CollectionUtils.isEmpty(companyList)){
            for (ErpOutOfStockOrderSearchListDTO d : dto) {
    ...
    public Integer saveOutOfStockOrder(AuthPlatformUserInfo userInfo, ErpOutOfStockOrderRQ rq) {
        ...
        List<ErpOutOfStockOrder> exist = selectList(new EntityWrapper<ErpOutOfStockOrder>().eq("code", code)
                .eq("company_id", companyId).eq("deleted", 0));
        // 新增校验编号重复
        if (ObjectUtils.isEmpty(rq.getId()) && !CollectionUtils.isEmpty(exist)) {
            log.info("单号编码重复，编码为:" + code);
            log.info("保存领料出库失败，单号编码重复，调用{}的{}方法出错", "ErpOutOfStockOrderServiceImpl", "saveOutOfStockOrder()");
        ...
        // 保存操作日志
        erpOperationLogService.saveLog(order.getCompanyId(),userInfo,order.getId(),"material_requisition",msg);
    ...
    public ErpOutOfStockOrderDTO getOutOfStockOrderById(Integer id) {
        ...
        List<AuthEnterpriseFeignDetailDTO> companyList = authEnterpriseFeignClient.getEnterpriseMoreDetail(Lists.newArrayList(erpOutOfStockOrder.getCompanyId())).getObject();
        if(!CollectionUtils.isEmpty(companyList)){
            erpOutOfStockOrder.setCompanyName(companyList.get(0).getName());
        }
        ...
        for (ErpOutOfStockOrderDetailDTO d : detailDTOList) {
            ...
            Integer productBatchId = d.getProductBatchId();
            ErpProductBatch pb = productBatchService.selectById(productBatchId);
