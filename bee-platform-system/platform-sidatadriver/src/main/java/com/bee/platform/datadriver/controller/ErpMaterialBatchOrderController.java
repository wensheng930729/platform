...
public class ErpMaterialBatchOrderController {
    ...
    @ApiOperation(value = "根据公司id查询公司下的料批信息")
    ...
    public ResponseResult<List<ErpProductAndBatchListDTO>> getMaterialBatchDetailProductList(@RequestParam Integer id) {
        if (ObjectUtils.isEmpty(id)) {
            log.info("参数异常");
            return ResponseResult.buildResponseResult(ResCodeEnum.ERROR_PARAMETER);
        }

        List<ErpProductAndBatchListDTO> dto = erpMaterialBatchOrderService.getMaterialBatchDetailProductList(id);
