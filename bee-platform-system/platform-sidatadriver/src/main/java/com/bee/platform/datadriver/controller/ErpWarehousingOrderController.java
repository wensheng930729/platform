...
public class ErpWarehousingOrderController {
    ...
    public ResponseResult<List<ErpWarehousingListDTO>> getUserWarehousingOrderList(HttpServletRequest request) {
        ...
        List<ErpWarehousingListDTO> dto = erpWarehousingOrderService.getUserWarehousingOrderList(companyId);
        return ResponseResult.buildResponseResult(ResCodeEnum.SUCCESS, dto);
    }
    ...
    public ResponseResult<ErpMaterialBatchProductDTO> getProductByMaterialBatchId(@RequestParam Integer id) {
        if (ObjectUtils.isEmpty(id)) {
            log.info("参数异常");
            return ResponseResult.buildResponseResult(ResCodeEnum.ERROR_PARAMETER);
        }

        ErpMaterialBatchProductDTO dto = erpWarehousingOrderService.getProductByMaterialBatchId(id);

        return ResponseResult.buildResponseResult(ResCodeEnum.SUCCESS, dto);

    }

    @ApiOperation(value = "根据化验单id查询出炉号id、炉号名称、班次、产成品id、产品名称、产品批次id、产品拼批次 、单位")
    @GetMapping("/getProductAndOtherByTestReportId")
    public ResponseResult<ProductAndOtherDTO> getProductAndOtherByTestReportId(@RequestParam Integer id) {
        if (ObjectUtils.isEmpty(id)) {
            log.info("参数异常");
            return ResponseResult.buildResponseResult(ResCodeEnum.ERROR_PARAMETER);
        }

        ProductAndOtherDTO dto = erpWarehousingOrderService.getProductAndOtherByTestReportId(id);
