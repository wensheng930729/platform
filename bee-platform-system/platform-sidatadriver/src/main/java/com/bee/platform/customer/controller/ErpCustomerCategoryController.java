...
public class ErpCustomerCategoryController {
    ...
    @ApiOperation(value = "登录用户企业-二级分类-下拉框", notes = "登录用户企业-二级分类-下拉框")
    ...
    public ResponseResult<List<ErpCustomerCategoryBoxDTO>> getUserTwoCategories(HttpServletRequest request){
        ...
        return customerCategoryService.getUserTwoCategories(userInfo,false);
    }

    @ApiOperation(value = "登录用户企业-二级分类-客户分类列表下拉框", notes = "登录用户企业-二级分类-客户分类列表下拉框")
    @GetMapping(value = "/user/lsit/secondCategory")
    public ResponseResult<List<ErpCustomerCategoryBoxDTO>> getUserTwoCategoriesList(HttpServletRequest request){
        String sysToken = request.getHeader("sysToken");
        AuthPlatformUserInfo userInfo = userFeignClient.simpleUserInfo(sysToken).getObject();
        if (ObjectUtils.isEmpty(userInfo)) {
            return ResponseResult.buildResponseResult(ResCodeEnum.NOT_FOUND_USERINFO);
        }
        return customerCategoryService.getUserTwoCategories(userInfo,true);
    }
