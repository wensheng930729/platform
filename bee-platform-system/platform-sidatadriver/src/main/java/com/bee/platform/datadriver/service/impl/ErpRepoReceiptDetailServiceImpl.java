...
public class ErpRepoReceiptDetailServiceImpl extends ServiceImpl<ErpRepoReceiptDetailMapper, ErpRepoReceiptDetail> implements ErpRepoReceiptDetailService {
    ...
    public List<RepoReceiptDetailDTO> listRepoReceiptDetail(ReceiptDetailRQ rq) {
        ...
        return this.getContractNum(repoReceiptDetail,Integer.valueOf(rq.getOrderId()));
    }

    private List<RepoReceiptDetailDTO> getContractNum(List<RepoReceiptDetailDTO> repoReceiptDetail, Integer id) {
        if (!org.springframework.util.CollectionUtils.isEmpty(repoReceiptDetail)) {
            //查询采购订单产品合同重量信息
            List<ErpPurchaseOrderDetailDTO> purchaseOrderDetails = purchaseOrderDetailMapper
                    .selectPurchaseDetailForNum(id);
            Map<Integer, BigDecimal> productNumMap = new HashMap<>();
            if (!org.springframework.util.CollectionUtils.isEmpty(purchaseOrderDetails)) {
                purchaseOrderDetails.forEach(order -> {
                    productNumMap.put(order.getBatchId(), order.getNum());
                });
            }
            //设置合同重量
            repoReceiptDetail.forEach(delivery -> {
    ...
    public Integer saveRepoReceiptDetailRawIn(AuthPlatformUserInfo userInfo, RepoReceiptDetailRawInRQ rq) {
        ...
        if(ObjectUtils.isEmpty(receiptId)){
            log.info("保存原料入库明细失败，没有关联的的原料入库主订单id 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailRawIn()");
        ...
        if(ObjectUtils.isEmpty(re)||ObjectUtils.isEmpty(re.getOrgId())){
            log.info("保存原料入库明细失败，没有找到关联的的原料入库主订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailRawIn()");
        ...
        if(ObjectUtils.isEmpty(erpPurchaseOrder)){
            log.info("保存原料入库明细失败，没有查到相关的源采购订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailRawIn()");
        ...
        List<AuthEnterpriseFeignDetailDTO> companyList = authEnterpriseFeignClient.getEnterpriseMoreDetail(Lists.newArrayList(companyId)).getObject();
    ...
    public void deleteRepoReceiptDetailRawIn(AuthPlatformUserInfo userInfo, Integer id) {
        ...
        if(ObjectUtils.isEmpty(receiptId)){
            log.info("删除原料入库明细失败，没有关联的的原料入库主订单id 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "deleteRepoReceiptDetailRawIn()");
            throw new BusinessException(ResCodeEnum.SAVE_FAIL, ExceptionMessageEnum.ERP_REPOSITORY_RECEIPT_RAW_IN_DETAIL_SAVE_FAILED_NO_MAIN_ID);
        }

        ErpRepositoryReceipt re = erpRepositoryReceiptService.selectById(receiptId);
        if(ObjectUtils.isEmpty(re)||ObjectUtils.isEmpty(re.getOrgId())){
            log.info("删除原料入库明细失败，没有找到关联的的原料入库主订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "deleteRepoReceiptDetailRawIn()");
            throw new BusinessException(ResCodeEnum.SAVE_FAIL, ExceptionMessageEnum.ERP_REPOSITORY_RECEIPT_RAW_IN_DETAIL_SAVE_FAILED_NO_MAIN_ORDER);
        }
        Integer companyId = re.getOrgId();
        Integer relatedOrderId = re.getRelatedOrderId();
        ...
        ErpPurchaseOrder erpPurchaseOrder = erpPurchaseOrderService.selectById(relatedOrderId);
        if(ObjectUtils.isEmpty(erpPurchaseOrder)){
            log.info("删除原料入库明细失败，没有查到相关的源采购订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "deleteRepoReceiptDetailRawIn()");
    ...
    public Integer saveRepoReceiptDetailProductOut(AuthPlatformUserInfo userInfo, RepoReceiptDetailProductOutRQ rq) {
        ...
        Integer receiptId = rq.getReceiptId();
        if(ObjectUtils.isEmpty(receiptId)){
            log.info("保存成品出库明细失败，没有关联的的成品出库主订单id 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailProductOut()");
        ...
        ErpRepositoryReceipt re = erpRepositoryReceiptService.selectById(receiptId);
        if(ObjectUtils.isEmpty(re)||ObjectUtils.isEmpty(re.getOrgId())){
            log.info("保存成品出库明细失败，没有找到关联的的成品出库主订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailProductOut()");
        ...
        if(ObjectUtils.isEmpty(erpSaleOrder)){
            log.info("保存成品出库明细失败，没有查到相关的源销售订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailProductOut()");
        ...
        ErpStock existStock = erpStockService.selectOne(wrapper);
        if(ObjectUtils.isEmpty(existStock)){
            log.info("保存成品出库明细失败,库存表里没有该产品，不能出库不存在的产品，调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailProductOut()");
            ...
            if(ObjectUtils.isEmpty(rqId)){
                ...
                if(outNum.compareTo(stockNum)>0){
                    log.info("保存成品出库明细失败,库存现有量小于出库数量，不能超额出库产品，调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "saveRepoReceiptDetailProductOut()");
    ...
    public void deleteRepoReceiptDetailProductOutById(AuthPlatformUserInfo userInfo, Integer id) {
        ...
        Integer receiptId = exist.getReceiptId();
        if(ObjectUtils.isEmpty(receiptId)){
            log.info("删除成品出库明细失败，没有关联的的成品出库主订单id 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "deleteRepoReceiptDetailProductOutById()");
        ...
        ErpRepositoryReceipt re = erpRepositoryReceiptService.selectById(receiptId);
        if(ObjectUtils.isEmpty(re)||ObjectUtils.isEmpty(re.getOrgId())){
            log.info("删除成品出库明细失败，没有找到关联的的成品出库主订单信息 调用{}的{}方法出错", "ErpRepoReceiptDetailServiceImpl", "deleteRepoReceiptDetailProductOutById()");
    ...
    public ErpRepoReceiptDetailRawInDTO getRepoReceiptDetailRawInById(Integer id) {
        ...
        ErpProductBatch pb = productBatchService.selectById(dto.getProductBatchId());
    ...
    public ErpRepoReceiptDetailProductOutDTO getRepoReceiptDetailProductOutById(Integer id) {
        ...
        ErpProduct p = productService.selectById(dto.getProductId());
        ErpProductBatch pb = productBatchService.selectById(dto.getProductBatchId());
        if (!ObjectUtils.isEmpty(p)) {
            dto.setProductName(p.getName()).setUnit(p.getUnit());
        }
