...
public class ErpSaleStatementServiceImpl extends ServiceImpl<ErpSaleStatementMapper, ErpSaleStatement> implements ErpSaleStatementService {
    ...
    public ResponseResult<ErpSaleStatementDTO> findStatementOrder(Integer id) {
        ...
        List<ErpSaleStatementDetailDTO> saleStatementDetailList = saleStmtDetailMapper.getSaleStatementsByStatementId(id);
        if (!CollectionUtils.isEmpty(saleStatementDetailList)) {
            ErpSaleStatementDetailDTO stmt = saleStatementDetailList.get(0);
            Integer productBatchId = stmt.getProductBatchId();
            if (ObjectUtils.isEmpty(productBatchId)) {
                productBatchId = 0;
            }
            saleStatementDTO.setBatchName(stmt.getBatchName()).setProductBatchId(productBatchId);
