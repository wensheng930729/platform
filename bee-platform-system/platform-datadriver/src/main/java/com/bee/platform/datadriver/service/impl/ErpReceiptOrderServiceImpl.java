...
public class ErpReceiptOrderServiceImpl extends ServiceImpl<ErpReceiptOrderMapper, ErpReceiptOrder> implements ErpReceiptOrderService {
    ...
    @Autowired
    private ErpReceiptOrderMapper erpReceiptOrderMapper;
    @Autowired
    ...
    public Integer saveReceiptOrder(AuthPlatformUserInfo userInfo, ErpReceiptOrderRQ rq) {
        ...
        for (ErpReceiptOrderDetailRQ dr : detail) {
            if(ObjectUtils.isEmpty(dr.getReceiptAmount())){
                log.error("详情单没有本次收款金额信息，不能创建订单");
        ...
        if (ObjectUtils.isEmpty(rq.getId()) && !CollectionUtils.isEmpty(exist)) {
            log.error("收款单号编码重复，编码为:" + code);
            log.error("保存销售收款单失败，收款单号编码重复，调用{}的{}方法出错", "ErpReceiptOrderServiceImpl", "saveReceiptOrder()");
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Integer updateReceiptOrder(AuthPlatformUserInfo userInfo, ErpReceiptOrderUpdateRQ rq) {
        BigDecimal amount = rq.getAmount();
        BigDecimal sum = rq.getSum();
        if (!ObjectUtils.isEmpty(amount) && !ObjectUtils.isEmpty(sum) && amount.compareTo(sum) != 0) {
            log.error("收款金额不等于本次收款金额合计，修改失败，调用{}的{}方法出错", "ErpMaterialBatchOrderServiceImpl", "updateMaterialBatchState()");
            throw new BusinessException(ResCodeEnum.UPDATA_FAIL, ExceptionMessageEnum.ERP_RECEIPT_ORDER_UPDATE_FAILED_MONEY_D);
        }

        Date time = new Date();
        Integer userId = userInfo.getId();
        ErpReceiptOrder properties = BeanUtils.copyProperties(rq, ErpReceiptOrder.class).setModifierId(userId).setModifyTime(time);
        if (!updateById(properties)) {
            log.error("修改销售收款单失败，调用{}的{}方法出错", "ErpReceiptOrderServiceImpl", "updateReceiptOrder()");
            throw new BusinessException(ResCodeEnum.UPDATA_FAIL, ExceptionMessageEnum.ERP_RECEIPT_ORDER_UPDATE_FAILED);
        }
    ...
    public ErpReceiptOrderDTO getReceiptOrderById(Integer id) {
        ...
        Integer companyId = erpReceiptOrder.getCompanyId();
        Integer customerId = erpReceiptOrder.getCustomerId();
        // 获取公司名称 客户名称
        String companyName = commonMapper.getCompanyNameById(companyId);
        String customerName = commonMapper.getCustomerNameById(customerId);
        erpReceiptOrder.setCompanyName(companyName).setCustomerName(customerName);
    ...
    public ResponseResult<List<ErpReceiptOrderSearchDTO>> searchReceiptByCondition(ErpReceiptSearchRQ rq, Page page, Integer companyId) {
        ...
        dto = baseMapper.searchReceiptOrderByCondition(rq,pagination);
