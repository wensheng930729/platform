...
public class ErpPurchaseOrderController{
    ...
    public ResponseResult<List<ErpPurchaseOrderInfoDTO>> listErpPurchaseOrder(HttpServletRequest request, @RequestBody OrderQueryRQ rq , Page page){
        ...
        // getEnterpriseFlatByCompanyId
        List<ErpPurchaseOrderInfoDTO> list = purchaseOrderService.listErpPurchaseOrder(pagination,rq,companyId);
    ...
    public ResponseResult<ErpPurchaseOrderDTO> getErpPurchaseOrder(HttpServletRequest request, String id){
    ...
    @PostMapping("/updateErpPurchaseOrder")
    @Log(businessType= EnumBusinessType.PURCHASE, operateType = OperateType.EDIT)
    @ApiOperation(value = "修改采购订单")
    public ResponseResult<Integer> updateErpPurchaseOrder(HttpServletRequest request, @RequestBody PurchaseOrderSaveRQ rq){
        AuthPlatformUserInfo userInfo = userInfoFeignClient
                .simpleUserInfo(WebUtils.getParam(ConstantsUtil.SYS_TOKEN, request))
                .getObject();
        if (ObjectUtils.isEmpty(userInfo)) {
    ...
    @PostMapping("/deleteErpPurchaseOrder")
    @Log(businessType= EnumBusinessType.PURCHASE, operateType = OperateType.DELETE)
    @ApiOperation(value = "删除采购订单信息")
    public ResponseResult<Integer> deleteErpPurchaseOrder(HttpServletRequest request, @RequestBody OrderDeleteRQ rq){
        AuthPlatformUserInfo userInfo = userInfoFeignClient
                .simpleUserInfo(WebUtils.getParam(ConstantsUtil.SYS_TOKEN, request))
                .getObject();
        if (ObjectUtils.isEmpty(userInfo)) {
        ...
        return purchaseOrderService.deleteErpPurchaseOrder(userInfo,rq);
    }
    ...
    public ResponseResult<OrderStatusCountDTO> countErpPurchaseOrderStatu(HttpServletRequest request){
        ...
        return purchaseOrderService.countErpPurchaseOrderStatu(companyId);
    }

    @GetMapping("/getErpPurchaseOrderInfoById")
    @ApiOperation(value = "根据采购订单合同id获取采购订单信息")
    public ResponseResult<ErpPurchaseOrderInfoDetailDTO> getErpPurchaseOrderByContractNo(HttpServletRequest request, Integer id){
        AuthPlatformUserInfo userInfo = userInfoFeignClient
                .simpleUserInfo(WebUtils.getParam(ConstantsUtil.SYS_TOKEN, request))
                .getObject();
        if (ObjectUtils.isEmpty(userInfo)) {
    ...
    public ResponseResult<List<ErpPucharseOrderBoxDTO>> getErpPurchaseOrderContractNoList(HttpServletRequest request){
        ...
        return purchaseOrderService.getPucharseOrderBox(userInfo,sysToken);
    }
