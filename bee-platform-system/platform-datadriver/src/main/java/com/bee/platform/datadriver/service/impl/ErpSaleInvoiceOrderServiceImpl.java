...
public class ErpSaleInvoiceOrderServiceImpl extends ServiceImpl<ErpSaleInvoiceOrderMapper, ErpSaleInvoiceOrder> implements ErpSaleInvoiceOrderService {
     ...
     */
    @Override
    public List<ErpSaleInvoiceOrderInfoDTO> listSaleInvoiceOrder(Pagination pagination, InvoiceOrderQueryRQ rq, Integer companyId) {
        ...
        if (org.springframework.util.CollectionUtils.isEmpty(enterpriseFlatDTOS)) {
            log.error("未查询到当前用户企业信息！ 类：{} 方法：{}", "ErpSaleInvoiceOrderServiceImpl", "listSaleInvoiceOrder");
        ...
        return saleInvoiceOrderMapper.selectInvoiceOrderByCondition(rq,pagination);
    }
    ...
    public ResponseResult<Integer> deleteSaleInvoiceOrder(AuthPlatformUserInfo userInfo, OrderDeleteRQ rq) {
        /*if (rq.getSubId() != null){
            ErpSaleInvoiceOrderDetail detail = invoiceOrderDetailMapper.selectById(new ErpSaleInvoiceOrderDetail()
            ...
            if (invoiceOrderDetailMapper.updateById(new ErpSaleInvoiceOrderDetail()
                ...
                log.error("删除采购发票明细失败,调用{}类{}方法出错","ErpSaleInvoiceOrderServiceImpl","deleteSaleInvoiceOrder()");
                throw new BusinessException(ResCodeEnum.DELETE_FAIL, ExceptionMessageEnum.ERP_SALE_INVOICE_DETAIL_DELETE_FAILED);
            }
            if (invoiceOrderDetailMapper.selectCount(new EntityWrapper<>(new ErpSaleInvoiceOrderDetail()
                ...
                if (saleInvoiceOrderMapper.updateById(new ErpSaleInvoiceOrder()
                    ...
                    log.error("删除采购发票失败,调用{}类{}方法出错","ErpSaleInvoiceOrderServiceImpl","deleteSaleInvoiceOrder()");
