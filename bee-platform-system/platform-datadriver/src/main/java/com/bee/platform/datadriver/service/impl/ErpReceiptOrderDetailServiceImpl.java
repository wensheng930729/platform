...
public class ErpReceiptOrderDetailServiceImpl extends ServiceImpl<ErpReceiptOrderDetailMapper, ErpReceiptOrderDetail> implements ErpReceiptOrderDetailService {
    ...
    public void deleteDetail(AuthPlatformUserInfo userInfo, Integer id) {
        ...
        if (ObjectUtils.isEmpty(exit)) {
            log.error("没有此详情单，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "deleteDetail()");
        ...
        if (ObjectUtils.isEmpty(erpSaleOrder)) {
            log.error("没有此销售订单，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "deleteDetail()");
...
//        }
        // 如果没有详单id  为新建 将本次收款金额 加到 销售单累计收款金额上
//        if(ObjectUtils.isEmpty(detailId)){
        ErpSaleOrder erpSaleOrder = erpSaleOrderService.selectById(saleOrderId);
        if (ObjectUtils.isEmpty(erpSaleOrder)) {
            log.error("没有此销售订单，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "saveReceiptOrderDetail()");
...
//        }
//        else {
//            // 有详情单id 为 修改  将本次收款金额 加到销售单累计收款金额上 再 减去 以前的本次收款金额
//            ErpReceiptOrderDetail detail = selectById(detailId);
//            if(ObjectUtils.isEmpty(detail)){
//                log.error("没有此收款详情单，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "saveReceiptOrderDetail()");
//                throw new BusinessException(ResCodeEnum.UPDATE_FAILED, ExceptionMessageEnum.ERP_RECEIPT_ORDER_DETAIL_SAVE_FAILED);
//            }
//
//            BigDecimal oldMoney = detail.getReceiptAmount();
//            ErpSaleOrder erpSaleOrder = erpSaleOrderService.selectById(saleOrderId);
//            if(ObjectUtils.isEmpty(erpSaleOrder)){
//                log.error("没有此销售订单，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "saveReceiptOrderDetail()");
//                throw new BusinessException(ResCodeEnum.UPDATE_FAILED, ExceptionMessageEnum.ERP_RECEIPT_ORDER_DETAIL_SAVE_FAILED);
//            }
//            erpSaleOrder.setTotalReceiptAmount(erpSaleOrder.getTotalReceiptAmount().add(nowMoney).subtract(oldMoney));
//            if(!erpSaleOrderService.updateById(erpSaleOrder)){
//                log.error("修改销售订单累计收款金额失败，调用{}的{}方法出错", "ErpReceiptOrderDetailServiceImpl", "saveReceiptOrderDetail()");
//                throw new BusinessException(ResCodeEnum.UPDATE_FAILED, ExceptionMessageEnum.ERP_RECEIPT_ORDER_DETAIL_SAVE_FAILED);
//            }
...
//        }
    ...
    public ErpReceiptOrderDetailDTO getReceiptOrderDetailById(Integer id) {
        ...
        ErpReceiptOrderDetailDTO dto = BeanUtils.copyProperties(detail, ErpReceiptOrderDetailDTO.class);
