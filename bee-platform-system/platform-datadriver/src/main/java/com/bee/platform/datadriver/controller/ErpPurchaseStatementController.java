...
import com.bee.platform.datadriver.rq.ErpPurchaseGoodsOrderSelectRQ;
...
public class ErpPurchaseStatementController {
    ...
    public ResponseResult<List<ErpPurchaseStatementDetailDTO>> findStatementList(HttpServletRequest request, @RequestBody()ErpPurchaseStatementRQ erpPurchaseStatementRQ, Page page) {
        ...
        if (Objects.isNull(companyId) && Objects.isNull(erpPurchaseStatementRQ.getCompanyId())){
    ...
    @ApiOperation(value = "根据采购合同id查询验收情况", notes = "根据采购合同id查询验收情况")
    @GetMapping("/getTestReportInfo")
    public ResponseResult findTestReportInfo(@RequestParam("id") Integer id) {
        return purchaseStatementService.findTestReportInfo(id);
    }
}
