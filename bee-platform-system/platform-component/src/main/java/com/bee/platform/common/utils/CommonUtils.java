...
public class CommonUtils {
     ...
     * 手机号默认截取 7位，保留4位
     */
    public static final int DEFAULT_CUT_OUT=7;

    /**
     * @notes: 手机号默认截取7位，保留4位
     * @Author: junyang.li
     * @Date: 17:21 2019/5/6
     * @param phoneNum : 手机号
     * @return: java.lang.String
     */
    public static String getPhoneTailNum(String phoneNum){
        return getPhoneTailNum(phoneNum,DEFAULT_CUT_OUT);
    }
     ...
     * @return: java.lang.String
     */
    public  static String getPhoneTailNum(String phoneNum,int cutOut){
        if(!Validator.isMobile(phoneNum)){
            throw new BusinessException(ResCodeEnum.NOT_PHONE_NUMBER,
                    ExceptionMessageEnum.USER_INVALID_MOBILE);
        }
        return phoneNum.substring(cutOut);
    }
    /**
     * @notes: 邮箱信息处理  如as***@qq.com
     * @Author: junyang.li
     * @Date: 10:14 2019/5/9
     * @param email : 待处理邮箱
     * @return: java.lang.String
    ...
    public  static String  treateMail(String email){
        if(!Validator.isEmail(email)){
            throw new BusinessException(ResCodeEnum.EMAIL_ERROR,
                    ExceptionMessageEnum.EMAIL_ERROR);
        }
        String[] str=email.split("@");
        StringBuilder builder=new StringBuilder(str[0].substring(0,2)).append("***@").append(str[1]);
        return builder.toString();
    }
