...
import com.bee.platform.user.authority.dao.mapper.AuthUsergroupUserBackMapper;
...
@Slf4j
@Service
public class AuthUsergroupUserBackServiceImpl extends ServiceImpl<AuthUsergroupUserBackMapper, AuthUsergroupUserBack> implements AuthUsergroupUserBackService {
    ...
    public void usergroupRalationUser(Integer usergroupId, List<Integer> userIds) {
        Wrapper<AuthUsergroupUserBack> wrapper = new EntityWrapper<AuthUsergroupUserBack>().eq("usergroup_id", usergroupId).eq("deleted", 0);
        // 查询用户组下是否有分配过的用户
        List<AuthUsergroupUserBack> usergroupUserList = selectList(wrapper);
        // 有旧的用户 删除旧数据
        if (!CollectionUtils.isEmpty(usergroupUserList)) {
            AuthUsergroupUserBack authUsergroupUser = new AuthUsergroupUserBack().setDeleted(1);
        ...
        if (!CollectionUtils.isEmpty(userIds)) {
            List<AuthUsergroupUserBack> list = Lists.newArrayList();
            for (Integer uId : userIds) {
                AuthUsergroupUserBack authUsergroupUser = new AuthUsergroupUserBack().setUsergroupId(usergroupId).setUserId(uId);
